...
var Map = cc.Layer.extend({
	...
	mapMoveByHeroPosition : function(position,type)
	{
	    ...
	    smap.mapMoveByHeroPosition(sdiff, 1);
...
function SMap() {
    ...
    this.init = function (HeroPosition, HeroMap) {
        ...
        var tr = this._map.getTextureRect();
        this._map.setPosition(cc.p(tr.width / 2, tr.height / 2));
    ...
    this.mapMoveByHeroPosition = function (diff, type) {
        if (type == true) {
            this._map.runAction(cc.MoveTo.create(0.2, diff));
        }
        else {
